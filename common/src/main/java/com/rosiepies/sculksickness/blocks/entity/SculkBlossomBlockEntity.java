package com.rosiepies.sculksickness.blocks.entity;
...
public class SculkBlossomBlockEntity extends BlockEntity implements TickableBlockEntity {
    ...
    public SculkBlossomBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(BlockEntityInit.SCULK_BLOSSOM_BLOCK_ENTITY.get(), blockPos, blockState);
    ...
    public void tick() {
        if (!returnedMaxSpewingTime) {
            ...
            if (SculkSickness.CONFIG.common.general.devLogs) SculkSickness.getLogger().info("Getting new maxSpewingTime! New maxSpewingTime is: " + maxSpewingTime);
        }
        if (!returnedMaxAgeTime) {
            returnedMaxAgeTime = true;
            maxAgeTime = getBloomInterval();
            if (SculkSickness.CONFIG.common.general.devLogs) SculkSickness.getLogger().info("Getting new maxAgeTime! New maxAgeTime is: " + maxAgeTime);
        }
        ...
        if (ageTime > maxAgeTime && !isSpewing() && blossomAge < 3) {
            ...
            SculkSickness.applyParticles(level.getServer().getLevel(level.dimension()), ParticleInit.SCULK_EFFECT.get(), Vec3.atCenterOf(this.worldPosition), new Vec3(0.5, 0, 0.5), 0.05F, 50, false, (Collection<ServerPlayer>) level.players());
            ...
            if (spewingTime < maxSpewingTime) {
                ...
                if (level.getServer() == null) return;
                SculkSickness.applyParticles(level.getServer().getLevel(level.dimension()), ParticleInit.SCULK_BLOSSOM_SPEWING.get(), Vec3.atCenterOf(this.worldPosition), new Vec3(0.125, 0.25, 0.125), 0F, 12, false, (Collection<ServerPlayer>) level.players());
                ...
                while (var11.hasNext()) {
                    ...
                    if (livingEntity.hasEffect(EffectInit.SCULK_SICKNESS.get()) || livingEntity.getType().is(TagInit.SCULK_IMMUNE)) return;
                ...
                blossomAge = 0;
                ageTime = 0;
                if (SculkSickness.CONFIG.common.general.devLogs) SculkSickness.getLogger().info("Resetting values to zero!");
